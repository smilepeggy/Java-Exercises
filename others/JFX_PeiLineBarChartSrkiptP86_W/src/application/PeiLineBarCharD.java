...
public class PeiLineBarCharD extends Application {
...
//	private ObservableList<PieChart.Data> pieCharDataList;
	...
	public void start(Stage primaryStage) {
		...
		// pie chart
		PieChart pieChartView = getPieChartView();
		// line chart
		LineChart<Number, Number> lineChartView = getLineChartView();

		// bar chart
		BarChart<String, Number> barChartView = getBarChartView();

		hBoxRoot.getChildren().addAll(pieChartView, lineChartView, barChartView);
	...
	private ArrayList<XYChart.Series<Number, Number>> readLineChartData(String filePath) {
		...
		try (BufferedReader reader = Files.newBufferedReader(Paths.get(filePath))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				competitionSeriesListTemp.setName(items[0]);
				...
				System.out.println("Series für den Verein " + items[0] + " erfolgreich importiert");
...
//	private ArrayList<Series<String, Number>> importLineChartData() { // from other people
